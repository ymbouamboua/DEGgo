...
All required dependencies are installed automatically when installing DEGgo.
...
Starting with DEGgo 0.1.0, users can analyze virtually any organism by supplying a custom Bioconductor-compatible OrgDb object through the orgdb argument.

This feature enables DEGgo workflows for:

* Bacterial transcriptomics (Staphylococcus aureus, Escherichia coli, etc.)
* Fungal species
...
Custom annotation databases can be generated using Bioconductor’s AnnotationForge package:
...
Once installed, the resulting OrgDb package can be supplied directly to DEGgo for gene annotation and Gene Ontology enrichment analyses.

Note: When organism = "custom", the orgdb argument is required. DEGgo will use the supplied annotation database throughout the annotation and enrichment workflow.


# Platform compatibility and HPC support

DEGgo is designed to provide a flexible RNA-seq downstream analysis workflow that can be used across different computing environments. Although DEGgo is suitable for HPC environments and large-scale transcriptomic analyses, it can also be run on standard workstations.

The package is fully compatible with Linux, macOS, and Windows systems as it is implemented entirely in R and relies on cross-platform CRAN and Bioconductor dependencies. No Linux-specific tools are required.

For HPC users, DEGgo can be integrated into batch computing workflows (e.g., SLURM) by running standard R scripts through `Rscript`. Output files, analysis parameters, and reproducibility information are automatically saved, facilitating pipeline tracking and automated execution.

DEGgo is also compatible with containerized environments such as Docker or Apptainer/Singularity, allowing reproducible deployment across HPC clusters and computational infrastructures.

This makes DEGgo accessible both to experienced bioinformaticians working on HPC systems and to R users performing RNA-seq analysis on local computers.
...
> Yvon MBOUAMBOUA, Vincent Prevot and Paolo Giacobini. DEGgo: an integrated framework for bulk RNA-seq differential expression analysis and functional enrichment.
