...
- [Supported organisms](#supported-organisms)
  ...
  - [Creating a custom OrgDb](#creating-a-custom-orgdb)
...
Starting with DEGgo 0.1.0, users can analyze virtually any organism by
supplying a custom Bioconductor-compatible OrgDb object through the
orgdb argument.

This feature enables DEGgo workflows for:

- Bacterial transcriptomics (Staphylococcus aureus, Escherichia coli,
  etc.)
- Fungal species
...
## Creating a custom OrgDb

Custom annotation databases can be generated using Bioconductor’s
...
Once installed, the resulting OrgDb package can be supplied directly to
DEGgo for gene annotation and Gene Ontology enrichment analyses.

Note: When organism = “custom”, the orgdb argument is required. DEGgo
will use the supplied annotation database throughout the annotation and
enrichment workflow.
